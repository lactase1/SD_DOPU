...
params.parallel.maxWorkers = 10;
% 并行相关资源限制: 最大可用内存 (GB) 用于一次性预加载阈值
params.parallel.maxMemGB = 20;
% 是否在函数结束时自动关闭并行池 (false = 保持池存活以节省启动时间)
params.parallel.autoClosePool = true;
...
if nargout == 0
    ...
    fprintf('生成 .dcm 文件 (params.generate_dcm): %s\n', iif(params.generate_dcm, '启用', '禁用'));
    fprintf('导出统计 .mat 文件 (params.save_mat): %s\n', iif(isfield(params,'save_mat') && params.save_mat, '启用', '禁用'));

...
function rPSOCT_process_single_file(varargin)
    ...
    % 确保在禁用 .dcm 输出时仍能保存 topLines/czrg（与早期代码保持一致）
    if ~params.processing.hasSeg
        save(fullfile(filepath, [name, '.mat']), 'topLines', 'czrg');
    end
    try
        saved_files = {};  % 记录已保存的文件用于日志输出
        target_frame_idx = 10;  % 手动指定 En-face 切片深度索引（可修改）

        % ------------------------------------------------------------------
        % 公共参数：与 DCM 生成保持完全一致的色彩映射参数
        %   PRRrg    - PhR 对比度范围（来自 params，与 DCM 中相同）
        %   rotAngle - OA quColoring 旋转角（与 DCM 中相同，固定为 440）
        %   cmap_phr - parula(256)，PhR colormap
        % ------------------------------------------------------------------
        PRRrg_stat  = params.polarization.PRRrg;  % PhR 对比度范围 [min, max]
        rotAngle_stat = 440;                       % OA HSV 色轮旋转角
        cmap_phr    = parula(256);                 % PhR 专用 colormap（256 级）
        ...
        if exist('OA_phys_flat', 'var') && ~isempty(OA_phys_flat)
            ...
            [nZ_oa, nX_oa, ~, nY_oa] = size(OA_phys_flat);  % [nZ, nX, 3, nY]
            frame_idx = min(max(target_frame_idx, 1), nZ_oa);

            % ---- 坐标对齐：转置为 [Y, X, 3]，与 DCM En-face 方向一致 ----
            % OA_phys_flat(frame_idx,:,:,:) → squeeze → [nX, 3, nY]
            % permute([nX,3,nY] → [nY,nX,3]) = [Y, X, 3]
            OA_enface_single = permute(squeeze(OA_phys_flat(frame_idx, :, :, :)), [3, 1, 2]);  % [Y, X, 3]

            % ---- 表面掩码：将展平后 surf_z 超出原始体积范围的"填充零"像素设为 NaN ----
            % 展平逻辑：z_src = frame_idx + surf_z - 1；若 surf_z > nZ_oa 则整列被 continue 跳过
            % 判据：topLines(ix,iy) > nZ_oa 代表该点从未被写入（初始化的 0，非真实信号）
            if exist('topLines', 'var') && ~isempty(topLines)
                % topLines 为 [nX, nY]，转置后为 [nY, nX] = [Y, X]，与 OA_enface_single 第1/2维对齐
                nan_mask_oa = topLines' > nZ_oa;  % [Y, X]，逻辑矩阵
                for ch = 1:3
                    tmp_ch = OA_enface_single(:, :, ch);
                    tmp_ch(nan_mask_oa) = NaN;
                    OA_enface_single(:, :, ch) = tmp_ch;
                end
                clear nan_mask_oa;
                fprintf('  已对 OA_enface_single 应用 topLines NaN 掩码\n');
            end

            fprintf('  -> OA_phys_flat   : [%d, %d, 3, %d]  (单精度浮点, 维度顺序: [Z, X, 3, Y])\n', nZ_oa, nX_oa, nY_oa);
            fprintf('  -> OA_enface_single: [%d, %d, 3]      (已对齐为 [Y, X, 3]，深度第 %d 层，NaN=无效像素)\n', nY_oa, nX_oa, frame_idx);

            % ---- 生成与 DCM 完全一致的 RGB 可视化矩阵（uint8 [Y, X, 3]）----
            % 逐 Y 列调用 quColoring，输入格式 [1, nX, 3]，输出 [1, nX, 3]
            % 最终拼合为 [Y, X, 3]，uint8，与 DCM 中 cumLA_cfg_hsv 生成逻辑完全一致
            fprintf('  生成 OA_enface_rgb (quColoring HSV)...\n');
            OA_enface_rgb = zeros(nY_oa, nX_oa, 3, 'uint8');
            for iy_stat = 1:nY_oa
                % OA_phys_flat(frame_idx, :, :, iy_stat) → [nX, 3]
                % reshape 为 [1, nX, 3] 供 quColoring 使用
                slice_qu = reshape(OA_phys_flat(frame_idx, :, :, iy_stat), [1, nX_oa, 3]);
                qu_out   = quColoring(slice_qu, rotAngle_stat);             % [1, nX, 3]
                OA_enface_rgb(iy_stat, :, :) = uint8(255 * reshape(qu_out, [nX_oa, 3]));
            end
            fprintf('  -> OA_enface_rgb  : [%d, %d, 3]  uint8 (quColoring HSV, [Y, X, 3]，与 DCM 完全一致)\n', nY_oa, nX_oa);

            save(oa_stats_filename, 'OA_phys_flat', 'OA_enface_single', 'OA_enface_rgb', '-v7.3');
            ...
            saved_files{end+1} = struct('name', '_OA_stats.mat', 'path', oa_stats_filename, 'size_mb', file_size_mb, 'description', 'OA物理数据[Z,X,3,Y] + 对齐En-face[Y,X,3](NaN掩码) + RGB可视化[Y,X,3]');

            % 内存保护：立即释放 OA 数据
            clear OA_phys_flat OA_enface_single OA_enface_rgb;
            fprintf('  已释放内存: OA_phys_flat, OA_enface_single, OA_enface_rgb\n');
        else
        ...
        if exist('PhR_phys_flat', 'var') && ~isempty(PhR_phys_flat)
            ...
            [nZ_phr, nX_phr, nY_phr] = size(PhR_phys_flat);  % [nZ, nX, nY]
            frame_idx = min(max(target_frame_idx, 1), nZ_phr);

            % ---- 坐标对齐：转置为 [Y, X]，与 DCM En-face 方向一致 ----
            % PhR_phys_flat(frame_idx, :, :) → squeeze → [nX, nY]
            % 转置 ' 得 [nY, nX] = [Y, X]
            PhR_enface_single = squeeze(PhR_phys_flat(frame_idx, :, :))';  % [Y, X]

            % ---- 表面掩码：将展平后超出原始数据范围的"填充零"像素设为 NaN ----
            % topLines(ix,iy) > nZ_phr 时该点在展平循环中被 continue 跳过，值为初始 0
            if exist('topLines', 'var') && ~isempty(topLines)
                % topLines' 为 [nY, nX] = [Y, X]，与 PhR_enface_single 维度完全对齐
                nan_mask_phr = topLines' > nZ_phr;  % [Y, X]
                PhR_enface_single(nan_mask_phr) = NaN;
                clear nan_mask_phr;
                fprintf('  已对 PhR_enface_single 应用 topLines NaN 掩码\n');
            end

            fprintf('  -> PhR_phys_flat   : [%d, %d, %d]  (单精度浮点, 维度顺序: [Z, X, Y])\n', nZ_phr, nX_phr, nY_phr);
            fprintf('  -> PhR_enface_single: [%d, %d]      (已对齐为 [Y, X]，深度第 %d 层，NaN=无效像素)\n', nY_phr, nX_phr, frame_idx);

            % ---- 生成与 DCM 完全一致的 RGB 可视化矩阵（uint8 [Y, X, 3]）----
            % 与 DCM 中 PRRc 生成逻辑完全一致：
            %   PRRc = uint8(ind2rgb(uint8(mat2gray(PhR, PRRrg)*256), parula(256)) * 256)
            % 注意：mat2gray 遇到 NaN 会输出 0（对应 parula 第 1 色），
            % 为使 NaN 区域在 RGB 中保持全黑（便于视觉辨别），先将 NaN 替换为 PRRrg(1)
            tmp_phr_rgb = PhR_enface_single;                          % [Y, X]，可能含 NaN
            tmp_phr_rgb(isnan(tmp_phr_rgb)) = PRRrg_stat(1);         % NaN → 下限值 → parula(1) ≈ 深蓝
            PhR_enface_rgb = uint8(ind2rgb(uint8(mat2gray(tmp_phr_rgb, PRRrg_stat) * 256), cmap_phr) * 256);
            % 结果为 [Y, X, 3]，uint8
            clear tmp_phr_rgb;
            fprintf('  -> PhR_enface_rgb  : [%d, %d, 3]  uint8 (parula色彩编码, [Y,X,3]，与 DCM 完全一致)\n', nY_phr, nX_phr);

            save(phr_stats_filename, 'PhR_phys_flat', 'PhR_enface_single', 'PhR_enface_rgb', '-v7.3');
            ...
            saved_files{end+1} = struct('name', '_PhR_stats.mat', 'path', phr_stats_filename, 'size_mb', file_size_mb, 'description', 'PhR物理数据[Z,X,Y] + 对齐En-face[Y,X](NaN掩码) + RGB可视化[Y,X,3]');

            % 内存保护：立即释放 PhR 数据
            clear PhR_phys_flat PhR_enface_single PhR_enface_rgb;
            fprintf('  已释放内存: PhR_phys_flat, PhR_enface_single, PhR_enface_rgb\n');
        else
    ...
    %% DCM到TIFF转换功能
    if isfield(params, 'tiff') && isfield(params.tiff, 'saveDicom') && params.tiff.saveDicom && isfield(params, 'generate_dcm') && params.generate_dcm && isfield(params.tiff, 'make_tiff') && params.tiff.make_tiff
        fprintf('\n开始DCM到TIFF转换...\n');
        try
